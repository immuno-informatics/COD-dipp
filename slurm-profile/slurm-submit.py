...
slurm_parser.add_argument("-o",
                          "--output",
                          help="file for batch script's standard output",
                          default="log.out" if "log.out" else None)
...
slurm_parser.add_argument("--wrap",
...
opt_keys = [
    "array", "account", "begin", "cpus_per_task", "dependency", "workdir",
    "error", "job_name", "mail_type", "mail_user", "ntasks", "nodes", "output",
    "partition", "quiet", "time", "wrap", "constraint", "mem", "gres"
]
...
if arg_dict["partition"] is None:
    if not "long":
        ...
        arg_dict["partition"] = "long"

# Set default account
if arg_dict["account"] is None:
    if "normal" != "":
        arg_dict["account"] = "normal"

filename = 'slurm-logs/' + job_properties['rule']
filename = filename + '-' + datetime.utcnow().strftime("%Y%m%d-%H%M%S.%f")
arg_dict['error'] = filename + '-' + arg_dict['error']
arg_dict['output'] = filename + '-' + arg_dict['output']
    ...
    if v is not None:
        ...
        if k == 'dependency' and v != '':
            if not isinstance(v, list):
                v = v.split(' ')
            if v != '':
                v = ','.join(["afterok:%s" % d for d in v])
        if k == 'job_name':
            v = v + ".{}.sh".format(job_properties["jobid"])
...
try:
    ...
    jobid = m.group(1)
    print(jobid)
    fh = open('slurm-logs/jobids.log', 'a')
    fh.write(filename + ':' + str(jobid) + '\n')
    fh.close
